...
export const sessionInitHandler: EventHandler = {
  async execute(input: NormalizedHookInput): Promise<HookResult> {
    ...
    if (!sessionId) {
      logger.warn(
        "HOOK",
        "session-init: No sessionId provided, skipping (Codex CLI or unknown platform)",
      );
      return {
        continue: true,
        suppressOutput: true,
    ...
    const prompt =
      !rawPrompt || !rawPrompt.trim() ? "[media prompt]" : rawPrompt;
    ...
    // Only initialize SDK agent for Claude Code (not Cursor or Gemini CLI)
    // Cursor and Gemini CLI don't use the SDK agent - they only need session/observation storage
    if (
      input.platform !== "cursor" &&
      input.platform !== "gemini-cli" &&
      sessionDbId
    ) {
      // Strip leading slash from commands for memory agent
      // /review 101 -> review 101 (more semantic for observations)
      const cleanedPrompt = prompt.startsWith("/")
        ? prompt.substring(1)
        : prompt;

      logger.debug(
        "HOOK",
        "session-init: Calling /sessions/{sessionDbId}/init",
        { sessionDbId, promptNumber },
      );
      ...
      logger.debug(
        "HOOK",
        "session-init: Skipping SDK agent init for Cursor platform",
        { sessionDbId, promptNumber },
      );
    }
