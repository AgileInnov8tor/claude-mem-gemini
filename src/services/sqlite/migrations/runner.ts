...
export class MigrationRunner {
  ...
  /**
   * Add lifecycle tracking columns for database-first stale reaping (migration 25)
   */
  private addSessionLifecycleColumns(): void {
    ...
    if (!hasLastActivity) {
      ...
      `);
      logger.debug("DB", "Added last_activity_epoch column to sdk_sessions table");
    }

    if (!hasReapedAt) {
      this.db.run("ALTER TABLE sdk_sessions ADD COLUMN reaped_at_epoch INTEGER");
      logger.debug("DB", "Added reaped_at_epoch column to sdk_sessions table");
    }

    if (!hasEndReason) {
      this.db.run("ALTER TABLE sdk_sessions ADD COLUMN end_reason TEXT");
      logger.debug("DB", "Added end_reason column to sdk_sessions table");
    }
    ...
    const needsStatusUpgrade =
      sdkSessionsSql?.sql?.includes("'stale'") !== true;

    if (needsStatusUpgrade) {
      this.db.run("BEGIN TRANSACTION");
      try {
        this.db.run(`
