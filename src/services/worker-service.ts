...
import {
  ...
  isGeminiSelected,
  isGeminiAvailable,
} from "./worker/GeminiAgent.js";
import {
  OpenRouterAgent,
  isOpenRouterSelected,
  isOpenRouterAvailable,
} from "./worker/OpenRouterAgent.js";
...
import {
  ...
  getProcessBySession,
  ensureProcessExit,
} from "./worker/ProcessRegistry.js";
...
export class WorkerService {
  ...
  private staleSessionReaperInterval: ReturnType<typeof setInterval> | null =
    null;
  ...
  constructor() {
    ...
    this.openRouterAgent = new OpenRouterAgent(
      this.dbManager,
      this.sessionManager,
    );
        ...
        return {
          ...
          lastInteraction: this.lastAiInteraction
            ? {
                ...
                ...(this.lastAiInteraction.error && {
                  error: this.lastAiInteraction.error,
                }),
              }
            : null,
        };
  ...
  private registerRoutes(): void {
    ...
    this.server.app.use("/api", async (req, res, next) => {
      ...
      try {
        ...
        logger.error(
          "HTTP",
          `Request to ${req.method} ${req.path} rejected — DB not initialized`,
          {},
          error as Error,
        );
        res.status(503).json({
  ...
  private async initializeBackground(): Promise<void> {
    try {
      ...
      const { SettingsDefaultsManager } =
        await import("../shared/SettingsDefaultsManager.js");
      ...
      if (chromaEnabled) {
        ...
        logger.info(
          "SYSTEM",
          "ChromaMcpManager initialized (lazy - connects on first use)",
        );
      } else {
        logger.info(
          "SYSTEM",
          "Chroma disabled via CLAUDE_MEM_CHROMA_ENABLED=false, skipping ChromaMcpManager",
        );
      }
      ...
      if (resetCount > 0) {
        logger.info(
          "SYSTEM",
          `Reset ${resetCount} stale processing messages to pending`,
        );
      }
      ...
      if (this.chromaMcpManager) {
        ChromaSync.backfillAllProjects()
          .then(() => {
            logger.info(
              "CHROMA_SYNC",
              "Backfill check complete for all projects",
            );
          })
          .catch((error) => {
            logger.error(
              "CHROMA_SYNC",
              "Backfill failed (non-blocking)",
              {},
              error as Error,
            );
          });
      ...
      this.staleSessionReaperInterval = setInterval(
        async () => {
          try {
            const result = await this.sessionManager.reapStaleSessions();
            for (const sessionDbId of result.autoSummarySessionIds) {
              this.sessionRoutes?.ensureSessionProcessing(
                sessionDbId,
                "stale-reap",
              );
            }
            const handled =
              result.reapedCount + result.autoSummarySessionIds.length;
            if (handled > 0) {
              logger.info("SYSTEM", `Reaped ${handled} stale sessions`);
            }
          } catch (e) {
            logger.error("SYSTEM", "Stale session reaper error", {
  ...
  private startSessionProcessor(
    ...
    if (session.abortController.signal.aborted) {
      logger.debug(
        "SYSTEM",
        "Replacing aborted AbortController before starting generator",
        {
    ...
    session.generatorPromise = agent
      ...
      .catch(async (error: unknown) => {
        ...
        if (
          unrecoverablePatterns.some((pattern) =>
          ...
          logger.error(
            "SDK",
            "Unrecoverable generator error - will NOT restart",
            {
              sessionId: session.sessionDbId,
        ...
        if (this.isSessionTerminatedError(error)) {
          logger.warn(
            "SDK",
            "SDK resume failed, falling back to standalone processing",
            {
              sessionId: session.sessionDbId,
              project: session.project,
        ...
        if (
          ...
          logger.warn(
            "SDK",
            "Detected stale resume failure, clearing memorySessionId for fresh start",
            {
              sessionId: session.sessionDbId,
      ...
      .finally(async () => {
        ...
        const {
          PendingMessageStore,
        } = require("./sqlite/PendingMessageStore.js");
        const pendingStore = new PendingMessageStore(
          this.dbManager.getSessionStore().db,
          3,
        );
        ...
        if (session.idleTimedOut) {
          logger.info(
            "SYSTEM",
            "Generator exited due to idle timeout, not restarting",
            {
              sessionId: session.sessionDbId,
            },
          );
        ...
        if (pendingCount > 0) {
          logger.info(
            "SYSTEM",
            "Pending work remains after generator exit, restarting with fresh AbortController",
            {
              sessionId: session.sessionDbId,
              pendingCount,
            },
  ...
  private async runFallbackForTerminatedSession(
    ...
    if (!session.memorySessionId) {
      ...
      this.dbManager
        .getSessionStore()
        .updateMemorySessionId(sessionDbId, syntheticId);
    }
    ...
    if (abandoned > 0) {
      logger.warn(
        "SDK",
        "No fallback available; marked pending messages abandoned",
        {
          sessionId: sessionDbId,
          abandoned,
        },
  ...
  async processPendingQueues(sessionLimit: number = 10): Promise<{
    ...
    // Use the same lifecycle threshold as the live stale reaper.
    const staleThreshold = Date.now() - SESSION_IDLE_TIMEOUT_MS;
    ...
    try {
      ...
      if (staleSessionIds.length > 0) {
        ...
        for (const sessionDbId of staleSessionIds) {
          ...
          if (activeSession?.generatorPromise) {
            continue;
          }
          ...
          if (
            !pendingStore.hasPendingSummarize(sessionDbId) &&
            !sessionStore.hasSessionSummary(sessionDbId)
          ) {
            this.sessionManager.prepareStaleAutoFinalize(
              sessionDbId,
              "startup_recovery",
            );
          ...
          failedMessageCount += pendingStore.markAllSessionMessagesAbandoned(
            sessionDbId,
          );
        ...
        if (finalizedCount + autoSummaryCount > 0) {
          logger.info(
            "SYSTEM",
            `Marked ${finalizedCount + autoSummaryCount} stale sessions`,
          );
        }
        ...
        if (failedMessageCount > 0) {
          logger.info(
            "SYSTEM",
            `Marked ${failedMessageCount} pending messages from stale sessions as failed`,
          );
        }
      }
    } catch (error) {
...
async function ensureWorkerStarted(port: number): Promise<boolean> {
  ...
  if (await waitForHealth(port, 1000)) {
    ...
    if (!versionCheck.matches) {
      ...
      if (isPidFileRecent(RESTART_COORDINATION_THRESHOLD_MS)) {
        logger.info(
          "SYSTEM",
          "Version mismatch detected but PID file is recent — another restart likely in progress, polling health",
          {
        ...
        const healthy = await waitForHealth(
          port,
          RESTART_COORDINATION_THRESHOLD_MS,
        );
        if (healthy) {
          logger.info(
            "SYSTEM",
            "Worker became healthy after waiting for concurrent restart",
          );
          return true;
        }
        logger.warn(
          "SYSTEM",
          "Worker did not become healthy after waiting — proceeding with own restart",
        );
      }

      logger.info(
        "SYSTEM",
        "Worker version mismatch detected - auto-restarting",
        {
          pluginVersion: versionCheck.pluginVersion,
          workerVersion: versionCheck.workerVersion,
        },
      ...
      const freed = await waitForPortFree(
        port,
        getPlatformTimeout(HOOK_TIMEOUTS.PORT_IN_USE_WAIT),
      );
      if (!freed) {
        logger.error(
          "SYSTEM",
          "Port did not free up after shutdown for version mismatch restart",
          { port },
        );
        return false;
      }
  ...
  const ready = await waitForReadiness(
    port,
    getPlatformTimeout(HOOK_TIMEOUTS.READINESS_WAIT),
  );
  if (!ready) {
    logger.warn(
      "SYSTEM",
...
async function main() {
  ...
  if (
    (hookInitiatedCommands.includes(command) || command === undefined) &&
  ...
  switch (command) {
    ...
    case "cursor": {
      ...
      const cursorResult = await handleCursorCommand(
        subcommand,
    ...
    case "hook": {
      ...
      if (!workerReady) {
        logger.warn(
          "SYSTEM",
          "Worker failed to start before hook, handler will retry",
        );
      }
      ...
      if (!platform || !event) {
        ...
        console.error(
          "Events: context, session-init, observation, summarize, session-complete",
        );
        process.exit(1);
      }
      ...
      if (!portInUse) {
        ...
        try {
          ...
          logger.failure(
            "SYSTEM",
            "Worker failed to start in hook",
            {},
            error as Error,
          );
          removePidFile();
    ...
    case "generate": {
      ...
      const { generateContextFile } =
        await import("../cli/context-file-commands.js");
      const result = await generateContextFile(dryRun);
      process.exit(result);
      break;
    }
    ...
    case "generate-gemini": {
      ...
      const { generateContextFile } =
        await import("../cli/context-file-commands.js");
      const result = await generateContextFile(dryRun, "GEMINI.md");
      process.exit(result);
      break;
    }
    ...
    case "clean": {
      const dryRun = process.argv.includes("--dry-run");
      const { cleanContextFiles } =
        await import("../cli/context-file-commands.js");
      const result = await cleanContextFiles(dryRun);
      process.exit(result);
      break;
    }
    ...
    default: {
      ...
      if (existingPidInfo && isProcessAlive(existingPidInfo.pid)) {
        logger.info(
          "SYSTEM",
          "Worker already running (PID alive), refusing to start duplicate",
          {
