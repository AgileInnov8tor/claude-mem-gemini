...
`+JSON.stringify(o,null,2):l=" "+this.formatData(o));let u="";if(n){let{sessionId:E,memorySessionId:g,correlationId:b,...m}=n;Object.keys(m).length>0&&(u=` {${Object.entries(m).map(([f,R])=>`${f}=${R}`).join(", ")}}`)}let T=`[${i}] [${a}] [${d}] ${c}${s}${u}${l}`;if(this.logFilePath)try{(0,C.appendFileSync)(this.logFilePath,T+`
...
`)[2]||"").match(/at\s+(?:.*\s+)?\(?([^:]+):(\d+):(\d+)\)?/),l=c?`${c[1].split("/").pop()}:${c[2]}`:"unknown",u={...s,location:l};return this.warn(e,`[HAPPY-PATH] ${t}`,u,n),o}},p=new W;var At={};function Rt(){return typeof __dirname<"u"?__dirname:(0,h.dirname)((0,ce.fileURLToPath)(At.url))}var Nt=Rt(),N=I.get("CLAUDE_MEM_DATA_DIR"),L=process.env.CLAUDE_CONFIG_DIR||(0,h.join)((0,de.homedir)(),".claude"),Vt=(0,h.join)(L,"plugins","marketplaces","thedotmack"),qt=(0,h.join)(N,"archives"),Kt=(0,h.join)(N,"logs"),Jt=(0,h.join)(N,"trash"),Qt=(0,h.join)(N,"backups"),zt=(0,h.join)(N,"modes"),Zt=(0,h.join)(N,"settings.json"),pe=(0,h.join)(N,"claude-mem.db"),es=(0,h.join)(N,"vector-db"),ts=(0,h.join)(N,"observer-sessions"),ss=(0,h.join)(L,"settings.json"),rs=(0,h.join)(L,"commands"),ns=(0,h.join)(L,"CLAUDE.md");function le(r){(0,_e.mkdirSync)(r,{recursive:!0})}function ue(){return(0,h.join)(Nt,"..")}var me=require("crypto");var Ct=3e4;function Ee(r,e,t){return(0,me.createHash)("sha256").update((r||"")+(e||"")+(t||"")).digest("hex").slice(0,16)}function Te(r,e,t){let s=t-Ct;return r.prepare("SELECT id, created_at_epoch FROM observations WHERE content_hash = ? AND created_at_epoch > ?").get(e,s)}var $=class{db;constructor(e=pe){e!==":memory:"&&le(N),this.db=new ge.Database(e),this.db.run("PRAGMA journal_mode = WAL"),this.db.run("PRAGMA synchronous = NORMAL"),this.db.run("PRAGMA foreign_keys = ON"),this.initializeSchema(),this.ensureWorkerPortColumn(),this.ensurePromptTrackingColumns(),this.removeSessionSummariesUniqueConstraint(),this.addObservationHierarchicalFields(),this.makeObservationsTextNullable(),this.createUserPromptsTable(),this.ensureDiscoveryTokensColumn(),this.createPendingMessagesTable(),this.renameSessionIdColumns(),this.repairSessionIdColumnRename(),this.addFailedAtEpochColumn(),this.addOnUpdateCascadeToForeignKeys(),this.addObservationContentHashColumn(),this.addSessionCustomTitleColumn(),this.addSessionPlatformColumn(),this.addSessionLifecycleColumns()}initializeSchema(){this.db.run(`
    ...
    `),this.db.run(`
      CREATE TABLE IF NOT EXISTS sdk_sessions (
        ...
        reaped_at_epoch INTEGER,
        end_reason TEXT,
        status TEXT CHECK(status IN ('active', 'completed', 'failed', 'stale')) NOT NULL DEFAULT 'active'
      );
      ...
      CREATE INDEX IF NOT EXISTS idx_sdk_sessions_status_activity ON sdk_sessions(status, last_activity_epoch DESC);
    ...
    `),this.db.run("CREATE INDEX IF NOT EXISTS idx_pending_messages_session ON pending_messages(session_db_id)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_pending_messages_status ON pending_messages(status)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_pending_messages_claude_session ON pending_messages(content_session_id)"),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(16,new Date().toISOString()),p.debug("DB","pending_messages table created successfully")}renameSessionIdColumns(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(17))return;p.debug("DB","Checking session ID columns for semantic clarity rename");let t=0,s=(n,o,i)=>{let a=this.db.query(`PRAGMA table_info(${n})`).all(),d=a.some(l=>l.name===o);return a.some(l=>l.name===i)?!1:d?(this.db.run(`ALTER TABLE ${n} RENAME COLUMN ${o} TO ${i}`),p.debug("DB",`Renamed ${n}.${o} to ${i}`),!0):(p.warn("DB",`Column ${o} not found in ${n}, skipping rename`),!1)};s("sdk_sessions","claude_session_id","content_session_id")&&t++,s("sdk_sessions","sdk_session_id","memory_session_id")&&t++,s("pending_messages","claude_session_id","content_session_id")&&t++,s("observations","sdk_session_id","memory_session_id")&&t++,s("session_summaries","sdk_session_id","memory_session_id")&&t++,s("user_prompts","claude_session_id","content_session_id")&&t++,this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(17,new Date().toISOString()),t>0?p.debug("DB",`Successfully renamed ${t} session ID columns`):p.debug("DB","No session ID column renames needed (already up to date)")}repairSessionIdColumnRename(){this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(19)||this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(19,new Date().toISOString())}addFailedAtEpochColumn(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(20))return;this.db.query("PRAGMA table_info(pending_messages)").all().some(n=>n.name==="failed_at_epoch")||(this.db.run("ALTER TABLE pending_messages ADD COLUMN failed_at_epoch INTEGER"),p.debug("DB","Added failed_at_epoch column to pending_messages table")),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(20,new Date().toISOString())}addOnUpdateCascadeToForeignKeys(){if(!this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(21)){p.debug("DB","Adding ON UPDATE CASCADE to FK constraints on observations and session_summaries"),this.db.run("PRAGMA foreign_keys = OFF"),this.db.run("BEGIN TRANSACTION");try{this.db.run("DROP TRIGGER IF EXISTS observations_ai"),this.db.run("DROP TRIGGER IF EXISTS observations_ad"),this.db.run("DROP TRIGGER IF EXISTS observations_au"),this.db.run("DROP TABLE IF EXISTS observations_new"),this.db.run(`
      ...
      `),this.db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='session_summaries_fts'").all().length>0&&this.db.run(`
          ...
          CREATE TRIGGER IF NOT EXISTS session_summaries_au AFTER UPDATE ON session_summaries BEGIN
            ...
            VALUES (new.id, new.request, new.investigated, new.learned, new.completed, new.next_steps, new.notes);
          END;
        `),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(21,new Date().toISOString()),this.db.run("COMMIT"),this.db.run("PRAGMA foreign_keys = ON"),p.debug("DB","Successfully added ON UPDATE CASCADE to FK constraints")}catch(t){throw this.db.run("ROLLBACK"),this.db.run("PRAGMA foreign_keys = ON"),t}}}addObservationContentHashColumn(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(22))return;this.db.query("PRAGMA table_info(observations)").all().some(n=>n.name==="content_hash")||(this.db.run("ALTER TABLE observations ADD COLUMN content_hash TEXT"),this.db.run("UPDATE observations SET content_hash = substr(hex(randomblob(8)), 1, 16) WHERE content_hash IS NULL"),this.db.run("CREATE INDEX IF NOT EXISTS idx_observations_content_hash ON observations(content_hash, created_at_epoch)"),p.debug("DB","Added content_hash column to observations table with backfill and index")),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(22,new Date().toISOString())}addSessionCustomTitleColumn(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(23))return;this.db.query("PRAGMA table_info(sdk_sessions)").all().some(n=>n.name==="custom_title")||(this.db.run("ALTER TABLE sdk_sessions ADD COLUMN custom_title TEXT"),p.debug("DB","Added custom_title column to sdk_sessions table")),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(23,new Date().toISOString())}addSessionPlatformColumn(){if(this.db.prepare("SELECT version FROM schema_versions WHERE version = ?").get(24))return;this.db.query("PRAGMA table_info(sdk_sessions)").all().some(n=>n.name==="platform")||(this.db.run("ALTER TABLE sdk_sessions ADD COLUMN platform TEXT DEFAULT 'claude-code'"),p.debug("DB","Added platform column to sdk_sessions table with default 'claude-code'")),this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(24,new Date().toISOString())}addSessionLifecycleColumns(){let e=this.db.query("PRAGMA table_info(sdk_sessions)").all(),t=e.some(i=>i.name==="last_activity_epoch"),s=e.some(i=>i.name==="reaped_at_epoch"),n=e.some(i=>i.name==="end_reason");if(t||this.db.run("ALTER TABLE sdk_sessions ADD COLUMN last_activity_epoch INTEGER"),s||this.db.run("ALTER TABLE sdk_sessions ADD COLUMN reaped_at_epoch INTEGER"),n||this.db.run("ALTER TABLE sdk_sessions ADD COLUMN end_reason TEXT"),this.db.run(`
      UPDATE sdk_sessions
      SET last_activity_epoch = COALESCE(last_activity_epoch, started_at_epoch)
      WHERE last_activity_epoch IS NULL
    `),this.db.prepare("SELECT sql FROM sqlite_master WHERE type = 'table' AND name = 'sdk_sessions'").get()?.sql?.includes("'stale'")!==!0){this.db.run("BEGIN TRANSACTION");try{this.db.run(`
        ...
        `),this.db.run(`
          INSERT INTO sdk_sessions_new (
            ...
            started_at, started_at_epoch, last_activity_epoch, completed_at,
            completed_at_epoch, reaped_at_epoch, end_reason, status,
            worker_port, prompt_counter, custom_title, platform
          )
          SELECT
            id, content_session_id, memory_session_id, project, user_prompt,
            started_at, started_at_epoch, COALESCE(last_activity_epoch, started_at_epoch), completed_at,
            completed_at_epoch, reaped_at_epoch, end_reason, status,
            worker_port, prompt_counter, custom_title, COALESCE(platform, 'claude-code')
          FROM sdk_sessions
        `),this.db.run("DROP TABLE sdk_sessions"),this.db.run("ALTER TABLE sdk_sessions_new RENAME TO sdk_sessions"),this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_claude_id ON sdk_sessions(content_session_id)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_sdk_id ON sdk_sessions(memory_session_id)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_project ON sdk_sessions(project)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_status ON sdk_sessions(status)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_started ON sdk_sessions(started_at_epoch DESC)"),this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_status_activity ON sdk_sessions(status, last_activity_epoch DESC)"),this.db.run("COMMIT")}catch(i){throw this.db.run("ROLLBACK"),i}}else this.db.run("CREATE INDEX IF NOT EXISTS idx_sdk_sessions_status_activity ON sdk_sessions(status, last_activity_epoch DESC)");this.db.prepare("INSERT OR IGNORE INTO schema_versions (version, applied_at) VALUES (?, ?)").run(25,new Date().toISOString())}updateMemorySessionId(e,t){this.db.prepare(`
      UPDATE sdk_sessions
      SET memory_session_id = ?, last_activity_epoch = ?
      WHERE id = ?
    `).run(t,Date.now(),e)}ensureMemorySessionIdRegistered(e,t){let s=this.db.prepare(`
    ...
    `).get(e)||null}getSdkSessionsBySessionIds(e){if(e.length===0)return[];let t=e.map(()=>"?").join(",");return this.db.prepare(`
      SELECT id, content_session_id, memory_session_id, project, user_prompt, custom_title,
             started_at, started_at_epoch, last_activity_epoch, completed_at,
             completed_at_epoch, reaped_at_epoch, end_reason, status, platform
      FROM sdk_sessions
      WHERE memory_session_id IN (${t})
      ORDER BY started_at_epoch DESC
    `).all(...e)}getPromptNumberFromUserPrompts(e){return this.db.prepare(`
      SELECT COUNT(*) as count FROM user_prompts WHERE content_session_id = ?
    `).get(e).count}createSDKSession(e,t,s,n,o){let i=new Date,a=i.getTime(),d=this.db.prepare(`
      SELECT id, status FROM sdk_sessions WHERE content_session_id = ?
    `).get(e);return d?(this.touchSessionActivity(d.id,a),d.status==="stale"&&this.reviveStaleSession(d.id,a),t&&this.db.prepare(`
          UPDATE sdk_sessions SET project = ?
          WHERE content_session_id = ? AND (project IS NULL OR project = '')
        `).run(t,e),n&&this.db.prepare(`
          UPDATE sdk_sessions SET custom_title = ?
          WHERE content_session_id = ? AND custom_title IS NULL
        `).run(n,e),o&&o!=="claude-code"&&this.db.prepare(`
          UPDATE sdk_sessions SET platform = ?
          WHERE content_session_id = ? AND (platform IS NULL OR platform = 'claude-code')
        `).run(o,e),d.id):(this.db.prepare(`
      ...
      VALUES (?, NULL, ?, ?, ?, ?, ?, ?, ?, 'active')
    `).run(e,t,s,n||null,o||"claude-code",i.toISOString(),a,a),this.db.prepare("SELECT id FROM sdk_sessions WHERE content_session_id = ?").get(e).id)}saveUserPrompt(e,t,s){let n=new Date,o=n.getTime(),a=this.db.prepare(`
      INSERT INTO user_prompts
      (content_session_id, prompt_number, prompt_text, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?)
    `).run(e,t,s,n.toISOString(),o);return this.db.prepare(`
      UPDATE sdk_sessions
      SET last_activity_epoch = ?
      WHERE content_session_id = ?
    `).run(o,e),a.lastInsertRowid}touchSessionActivity(e,t=Date.now()){this.db.prepare(`
      UPDATE sdk_sessions
      SET last_activity_epoch = ?
      WHERE id = ?
    `).run(t,e)}reviveStaleSession(e,t=Date.now()){this.db.prepare(`
      UPDATE sdk_sessions
      SET
        status = 'active',
        completed_at = NULL,
    ...
    `).run(t,e)}finalizeSession(e,t,s,n=Date.now()){return this.db.prepare(`
      UPDATE sdk_sessions
      SET
        status = ?,
        completed_at = ?,
        completed_at_epoch = ?,
        reaped_at_epoch = CASE WHEN ? = 'stale' THEN ? ELSE reaped_at_epoch END,
        end_reason = ?,
        last_activity_epoch = ?
      WHERE id = ? AND status = 'active'
    `).run(t,new Date(n).toISOString(),n,t,t==="stale"?n:null,s||null,n,e).changes>0}getStaleActiveSessionIds(e){return this.db.prepare(`
      SELECT id
      FROM sdk_sessions
      WHERE status = 'active' AND last_activity_epoch < ?
      ORDER BY last_activity_epoch ASC
    `).all(e).map(s=>s.id)}hasSessionSummary(e){return this.db.prepare(`
      SELECT COUNT(*) AS count
      FROM session_summaries ss
      JOIN sdk_sessions s ON s.memory_session_id = ss.memory_session_id
      WHERE s.id = ?
    `).get(e).count>0}getUserPrompt(e,t){return this.db.prepare(`
      SELECT prompt_text
      FROM user_prompts
      WHERE content_session_id = ? AND prompt_number = ?
      LIMIT 1
    `).get(e,t)?.prompt_text??null}storeObservation(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),c=Ee(e,s.title,s.narrative),l=Te(this.db,c,a);if(l)return{id:l.id,createdAtEpoch:l.created_at_epoch};let T=this.db.prepare(`
      INSERT INTO observations
      (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
       files_read, files_modified, prompt_number, discovery_tokens, content_hash, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.type,s.title,s.subtitle,JSON.stringify(s.facts),s.narrative,JSON.stringify(s.concepts),JSON.stringify(s.files_read),JSON.stringify(s.files_modified),n||null,o,c,d,a);return this.db.prepare(`
      UPDATE sdk_sessions
      SET last_activity_epoch = ?
      WHERE memory_session_id = ?
    `).run(a,e),{id:Number(T.lastInsertRowid),createdAtEpoch:a}}storeSummary(e,t,s,n,o=0,i){let a=i??Date.now(),d=new Date(a).toISOString(),l=this.db.prepare(`
      INSERT INTO session_summaries
      (memory_session_id, project, request, investigated, learned, completed,
       next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(e,t,s.request,s.investigated,s.learned,s.completed,s.next_steps,s.notes,n||null,o,d,a);return this.db.prepare(`
      UPDATE sdk_sessions
      SET last_activity_epoch = ?
      WHERE memory_session_id = ?
    `).run(a,e),{id:Number(l.lastInsertRowid),createdAtEpoch:a}}storeObservations(e,t,s,n,o,i=0,a){let d=a??Date.now(),c=new Date(d).toISOString();return this.db.transaction(()=>{let u=[],T=this.db.prepare(`
        INSERT INTO observations
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
      ...
      `);for(let g of s){let b=T.run(e,t,g.type,g.title,g.subtitle,JSON.stringify(g.facts),g.narrative,JSON.stringify(g.concepts),JSON.stringify(g.files_read),JSON.stringify(g.files_modified),o||null,i,c,d);u.push(Number(b.lastInsertRowid))}let E=null;if(n){let b=this.db.prepare(`
        ...
        (memory_session_id, project, type, title, subtitle, facts, narrative, concepts,
         files_read, files_modified, prompt_number, discovery_tokens, created_at, created_at_epoch)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `);for(let S of s){let f=g.run(e,t,S.type,S.title,S.subtitle,JSON.stringify(S.facts),S.narrative,JSON.stringify(S.concepts),JSON.stringify(S.files_read),JSON.stringify(S.files_modified),a||null,d,u,l);E.push(Number(f.lastInsertRowid))}let b;if(n){let f=this.db.prepare(`
          INSERT INTO session_summaries
          (memory_session_id, project, request, investigated, learned, completed,
           next_steps, notes, prompt_number, discovery_tokens, created_at, created_at_epoch)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    `).all(...c)}getTimelineAroundTimestamp(e,t=10,s=10,n){return this.getTimelineAroundObservation(null,e,t,s,n)}getTimelineAroundObservation(e,t,s=10,n=10,o){let i=o?"AND project = ?":"",a=o?[o]:[],d,c;if(e!==null){let m=`
      ...
      `,S=`
        ...
        ORDER BY id ASC
        LIMIT ?
      `;try{let f=this.db.prepare(m).all(e,...a,s+1),R=this.db.prepare(S).all(e,...a,n+1);if(f.length===0&&R.length===0)return{observations:[],sessions:[],prompts:[]};d=f.length>0?f[f.length-1].created_at_epoch:t,c=R.length>0?R[R.length-1].created_at_epoch:t}catch(f){return p.error("DB","Error getting boundary observations",void 0,{error:f,project:o}),{observations:[],sessions:[],prompts:[]}}}else{let m=`
      ...
      `,S=`
        ...
        ORDER BY created_at_epoch ASC
        LIMIT ?
      `;try{let f=this.db.prepare(m).all(t,...a,s),R=this.db.prepare(S).all(t,...a,n+1);if(f.length===0&&R.length===0)return{observations:[],sessions:[],prompts:[]};d=f.length>0?f[f.length-1].created_at_epoch:t,c=R.length>0?R[R.length-1].created_at_epoch:t}catch(f){return p.error("DB","Error getting boundary timestamps",void 0,{error:f,project:o}),{observations:[],sessions:[],prompts:[]}}}let l=`
      SELECT *
      FROM observations
...
`}function st(r,e,t,s){let n=[];return s?n.push(...He(r)):n.push(...ve(r)),s?n.push(...Be()):n.push(...ye()),s?n.push(...We()):n.push(...De()),s?n.push(...Ye()):n.push(...Me()),P(t)&&(s?n.push(...Ve(e,t)):n.push(...Ue(e,t))),n}var ee=y(require("path"),1);function H(r){if(!r)return[];try{let e=JSON.parse(r);return Array.isArray(e)?e:[]}catch(e){return p.debug("PARSER","Failed to parse JSON array, using empty fallback",{preview:r?.substring(0,50)},e),[]}}function nt(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",hour:"numeric",minute:"2-digit",hour12:!0})}function ot(r){return new Date(r).toLocaleString("en-US",{hour:"numeric",minute:"2-digit",hour12:!0})}function it(r){return new Date(r).toLocaleString("en-US",{month:"short",day:"numeric",year:"numeric"})}function rt(r,e){return ee.default.isAbsolute(r)?ee.default.relative(e,r):r}function at(r,e,t){let s=H(r);if(s.length>0)return rt(s[0],e);if(t){let n=H(t);if(n.length>0)return rt(n[0],e)}return"General"}function yt(r){let e=new Map;for(let s of r){let n=s.type==="observation"?s.data.created_at:s.data.displayTime,o=it(n);e.has(o)||e.set(o,[]),e.get(o).push(s)}let t=Array.from(e.entries()).sort((s,n)=>{let o=new Date(s[0]).getTime(),i=new Date(n[0]).getTime();return o-i});return new Map(t)}function Dt(r,e){return e.fullObservationField==="narrative"?r.narrative:r.facts?H(r.facts).join(`
`):null}function Mt(r,e,t,s,n,o){let i=[];o?i.push(...qe(r)):i.push(...ke(r));let a=null,d="",c=!1;for(let l of e)if(l.type==="summary"){c&&(i.push(""),c=!1,a=null,d="");let u=l.data,T=nt(u.displayTime);o?i.push(...ze(u,T)):i.push(...Fe(u,T))}else{let u=l.data,T=at(u.files_modified,n,u.files_read),E=ot(u.created_at),g=E!==d,b=g?E:"";d=E;let m=t.has(u.id);if(T!==a&&(c&&i.push(""),o?i.push(...Ke(T)):i.push(...xe(T)),a=T,c=!0),m){let S=Dt(u,s);o?i.push(...Qe(u,E,g,S,s)):(c&&!o&&(i.push(""),c=!1),i.push(...$e(u,b,S,s)),a=null)}else o?i.push(Je(u,E,g,s)):i.push(we(u,b,s))}return c&&i.push(""),i}function dt(r,e,t,s,n){let o=[],i=yt(r);for(let[a,d]of i)o.push(...Mt(a,d,e,t,s,n));return o}function _t(r,e,t){return!(!r.showLastSummary||!e||!!!(e.investigated||e.learned||e.completed||e.next_steps)||t&&e.created_at_epoch<=t.created_at_epoch)}function ct(r,e){let t=[];return e?(t.push(...k("Investigated",r.investigated,_.blue)),t.push(...k("Learned",r.learned,_.yellow)),t.push(...k("Completed",r.completed,_.green)),t.push(...k("Next Steps",r.next_steps,_.magenta))):(t.push(...U("Investigated",r.investigated)),t.push(...U("Learned",r.learned)),t.push(...U("Completed",r.completed)),t.push(...U("Next Steps",r.next_steps))),t}function pt(r,e){return e?Ze(r):Pe(r)}function lt(r,e,t){return!P(e)||r.totalDiscoveryTokens<=0||r.savings<=0?[]:t?et(r.totalDiscoveryTokens,r.totalReadTokens):Xe(r.totalDiscoveryTokens,r.totalReadTokens)}var Ut=ut.default.join((0,mt.homedir)(),".claude","plugins","marketplaces","thedotmack","plugin",".install-version");function kt(){try{return new $}catch(r){if(r.code==="ERR_DLOPEN_FAILED"){try{(0,Et.unlinkSync)(Ut)}catch(e){p.debug("SYSTEM","Marker file cleanup failed (may not exist)",{},e)}return p.error("SYSTEM","Native module rebuild needed - restart Claude Code to auto-fix"),null}throw r}}function xt(r,e){return e?tt(r):je(r)}function wt(r,e,t,s,n,o,i){let a=[],d=K(e);a.push(...st(r,d,s,i));let c=t.slice(0,s.sessionCount),l=Ce(c,t),u=Z(e,l),T=Ie(e,s.fullObservationCount);a.push(...dt(u,T,s,n,i));let E=t[0],g=e[0];_t(s,E,g)&&a.push(...ct(E,i));let b=z(e,s,o,n);return a.push(...pt(b,i)),a.push(...lt(d,s,i)),a.join(`
`).trimEnd()}async function te(r,e=!1){let t=Y(),s=r?.cwd??process.cwd(),n=Se(s),o=r?.projects||[n],i=kt();if(!i)return"";try{let a=o.length>1?Ne(i,o,t):J(i,n,t),d=o.length>1?Ae(i,o,t):Q(i,n,t);return a.length===0&&d.length===0?xt(n,e):wt(n,a,d,t,s,r?.session_id,e)}finally{i.close()}}0&&(module.exports={generateContext});
